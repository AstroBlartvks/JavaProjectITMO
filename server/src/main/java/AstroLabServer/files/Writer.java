...
public class Writer {
     ...
     * @throws Exception if the environment variable
     *      is not found or if an error occurs while writing to the file.
     */
    public void writeToEnv(CustomCollection collection) throws Exception {
        String path = System.getenv(JavaFileStrings.JAVAEE);

        if (path == null) {
            throw new IllegalStateException("The path to the file in the environment variable '" +
                                            JavaFileStrings.JAVAEE + "' was not found!");
        }
